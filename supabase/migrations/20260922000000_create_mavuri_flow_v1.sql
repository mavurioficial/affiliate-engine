...
create table if not exists public.flow_clicks (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, offer_id uuid references public.flow_offers(id) on delete set null, channel_id uuid references public.flow_channels(id) on delete set null, tracking_id text, click_count integer not null default 0, last_clicked_at timestamptz, created_at timestamptz not null default now());
